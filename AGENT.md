...
This repository contains reusable GitHub Actions workflows and composite actions (Astro, Vite, Fixer) for the AREA44 ecosystem.
...
- **Compatibility**: Preserve original `action.yml` files in root directories (astro/, fixer/, vite/) and maintain original public inputs/outputs for backward compatibility.
...
- **Fixer Logic**: Core logic for running lint/format scripts is in `src/fixer.ts` (function `executeAutofix`).

## Coding Preferences

- Use descriptive names for internal functions (e.g., `setOutputs`, `executeAutofix`) rather than generic names.
