...
This repository contains reusable GitHub Actions workflows and composite actions (Astro, Vite, Autofix, Images) for the AREA44 ecosystem.
...
- **Compatibility**: Preserve original `action.yml` files in root directories (astro/, autofix/, images/, vite/) and maintain original public inputs/outputs for backward compatibility.
...
- **Lint/Format Logic**: Core logic for running lint/format scripts is implemented directly in `autofix/action.yml` and `images/action.yml` using bash.
